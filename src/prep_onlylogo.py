...
def main(path_data, path_data_lld, path_data_metu, path_lbls_metu, resize_size, path_out, batch_size):
    ...
    for paths_, resize in zip(np.array_split(paths, batch_size), np.array_split(resize_size_, batch_size)):
        ...
        print("getting images of ONLYLOGO")
        imgs = np.array([d[0] for i,d in enumerate(data)], dtype="uint8")
        print("getting texts of ONLYLOGO")
        txts = [d[1] for i,d in enumerate(data)]
        print("getting colors of ONLYLOGO")
        colors = np.array([d[2] for i,d in enumerate(data)], dtype="float32")
        print("getting OH-colors of ONLYLOGO")
        colors_old = np.array([d[3] for i,d in enumerate(data)], dtype="float32")
        print(f"saving to part to {path_out}onlylogo_prep_pt{batch_nr}.npz")
        np.savez_compressed(os.path.join(path_out, f"onlylogo_prep_pt{batch_nr}.npz"),
    ...
    for i in range(batch_nr):
        data = np.load(os.path.join(path_out, f"onlylogo_prep_pt{i}.npz"), allow_pickle=True)
        for j in tqdm(data["imgs"]):
            imsave(os.path.join(path_out, f"only_logo/{id_nr}.png"), j)
            id_nr+=1
    ...
    np.savez_compressed(os.path.join(path_out, f"onlylogo_conditionals.npz"),
                        colors=colors,
                        txts=txts,
                        txt_embs = txt_embs,
                        colors_old=np.array(colors_old))
    print(f"Done: saved all images in {path_out}imgs/ and the conditionals as onlylogo_conditionals.npz")
    ...
    with open(path) as fd:
        try:
            ...
            if infos.get("MarkFeature")=="Figurative":
                ...
                #                                     for x in nice_infos}
                if not infos.get("WordMarkSpecification", {}).get("MarkVerbalElementText"):
...
if __name__=="__main__":
    ...
    if not os.path.isdir(os.path.join(path_out, "only_logo")):
        os.mkdir(os.path.join(path_out, "only_logo"))
