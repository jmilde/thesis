...
def get_activations(image_paths, sess, batch_size=50, verbose=False):
    ...
    for i in tqdm(range(n_batches)):
        ...
        batch = [imread(path) for path in image_paths[start:end]]
...
def get_activations_from_files(files, sess, batch_size=50, verbose=False):
    ...
    for i in range(n_batches):
        ...
        pred = sess.run(inception_layer, {'ExpandDims:0': batch}) #FID_Inception_Net/ExpandDims:0
        pred_arr[start:end] = pred.reshape(batch_size,-1)
